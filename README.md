...
- ログイン必須の商品管理
- 仕入れ判断シミュレーター
  - 想定利益、利益率、ROI、上限仕入価格、必要売価USDを計算
  - 目標利益率と目標ROIでOK / 注意 / 見送りを判定
  - 判定結果から商品登録画面へ入力値を引き継ぎ
- セラー設定
  - デフォルト目標利益率、目標ROI、送料、為替、eBay手数料率を保存
  - 仕入れ判断と商品登録の初期値に反映
- 商品の登録、一覧、編集、削除
- 商品詳細ページ
- 一覧からのクイック更新
- ステータス管理: 仕入れ済み / 出品中 / 売却済み
- 想定利益、実利益、想定との差額、ROIの自動計算
- 仕入れ日、出品日、売却日による販売日数・在庫日数の管理
- SKU、状態、数量、URL、販売先国の記録
- 商品画像URLによるサムネイル表示
- CSVインポート / エクスポート
- 実績分析ダッシュボード
  - 月別実利益グラフ
